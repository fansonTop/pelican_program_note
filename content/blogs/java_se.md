...
2. 包装类有一个非常实用的功能，就是将字符串解析为对应的基本类型，前提，该字符串的内容正确的描述了基本类型可以保存的值
...
2. finally块：finall块是异常处理机制的最后一块，可以保证只要代码执行到try当中，无论是否抛出异常，finally块中的代码都必定执行，因此我们经常将释放资源这类操作放在这里
